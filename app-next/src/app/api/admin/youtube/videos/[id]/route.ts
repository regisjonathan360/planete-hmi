...
import { EXCLUDED_YOUTUBE_VIDEO_TYPES } from "@/lib/youtube/constants";
import { toSafeApiError } from "@/lib/youtube/api-error";

export const dynamic = "force-dynamic";

const EXCLUDED_SET = new Set<string>(EXCLUDED_YOUTUBE_VIDEO_TYPES);
...
export async function PATCH(
  ...
  try {
    ...
    if (input.isEligible && !input.trackId) {
      return NextResponse.json(
        { error: { code: "validation_error", message: "Une vidéo éligible doit être associée à une chanson." } },
        { status: 400 }
      );
    }
    if (input.isEligible && EXCLUDED_SET.has(input.videoType)) {
      return NextResponse.json(
        { error: { code: "validation_error", message: "Ce type de vidéo ne peut pas être éligible au classement principal." } },
        { status: 400 }
      );
    }
    if ((input.reviewStatus === "EXCLUDED" || (input.reviewStatus === "APPROVED" && !input.isEligible))
    ...
    if (!row?.success) {
      const msgMap: Record<string, { msg: string; status: number }> = {
        ...
        incoherent_eligibility: { msg: "Combinaison statut, type, chanson et éligibilité incohérente.", status: 400 },
