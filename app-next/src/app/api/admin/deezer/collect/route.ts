...
export async function POST() {
  ...
  return NextResponse.json({
    ...
    excludedArtists: draft.excluded,
    message: `Collecte Deezer réussie : ${draft.imported} musiques, ${data.summary.distinctArtists} artistes.${draft.excluded ? ` ${draft.excluded} entrée(s) d’artistes exclus ignorée(s).` : ""}${duplicatesFound ? ` ${duplicatesFound} doublon(s) détecté(s).` : ""}`,
  });
}
