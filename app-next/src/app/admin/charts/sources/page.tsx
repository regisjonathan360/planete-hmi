...
export default async function SourcesPage() {
  ...
  const { data: sources } = await supabase
    .from("chart_sources")
    .select("source_key, display_name, platform, chart_context, ingestion_mode, is_enabled, is_automatic")
    .order("platform");
  ...
  return (
    <>
      ...
      <table>
        <thead>
          <tr><th>Plateforme</th><th>Classement</th><th>Contexte</th><th>Mode</th><th>Auto</th><th>Actif</th></tr>
        </thead>
        <tbody>
          {(sources ?? []).map((s) => (
            <tr key={s.source_key}>
