...
describe("PATCH /api/admin/youtube/videos/[id]", () => {
  ...
  it("retourne 400 si éligible sans chanson", async () => {
    const res = await PATCH(
      makeRequest("PATCH", {
    ...
    // The D10 schema rejects this combination (trackId="" transforms to null, then superRefine fails)
    expect(res.status).toBe(400);
  });

  it("retourne 400 si SHORT éligible", async () => {
    const res = await PATCH(
      makeRequest("PATCH", {
