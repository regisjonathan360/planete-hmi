import { ELIGIBLE_YOUTUBE_VIDEO_TYPE_SET } from "./constants";
import type {
  RankedYouTubeTrack,
  YouTubeMetricValues,
...
export function calculateYouTubeVideoPerformance(
  ...
  if (!video.trackId) return ineligiblePerformance(input, "TRACK_MISSING");
  if (video.verificationStatus !== "APPROVED") {
    return ineligiblePerformance(input, "VIDEO_NOT_APPROVED");
  }
  if (video.videoType === "SHORT") {
    return ineligiblePerformance(input, "SHORT_EXCLUDED");
  }
  if (
    video.eligibilityStatus !== "ELIGIBLE" ||
    !ELIGIBLE_YOUTUBE_VIDEO_TYPE_SET.has(video.videoType)
  ) {
