...
/** Son d'erreur (servi depuis /public/games/solitaire par Next). */
const ERROR_SOUND_SRC = "/games/solitaire/ding.mp3";
    ...
    drop: (item, monitor) => {
      ...
      const calculateWindowPosition = (
        ...
        const windowAxisPosition =
          maxPosibleAxisPosition < (window as unknown as Record<string, number>)[windowAxis]
            ? windowPositon + differenceInPosition
            : ((window as unknown as Record<string, number>)[windowAxis] as number) -
              parsedWindowSize;
