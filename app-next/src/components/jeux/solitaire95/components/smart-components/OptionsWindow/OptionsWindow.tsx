...
> = (props) => {
  ...
  const onOkClick = useCallback(() => {
    ...
    if (backgroundId !== (storedBackground?.id ?? SOLITAIRE_CLASSIC_BACKGROUND_ID)) {
      setStoredSolitaireBackground(getSolitaireBackgroundById(backgroundId));
    }
  }, [
    toggleOptionsWindow,
    toggleBottomBar,
  ...
  return (
    ...
    >
      ...
      <div className={styles.backgroundSection}>
        <p className={styles.backgroundSection__label}>Background</p>
        <div className={styles.backgroundSection__grid}>
          <button
            type="button"
            onClick={() => setBackgroundId(SOLITAIRE_CLASSIC_BACKGROUND_ID)}
            className={
              backgroundId === SOLITAIRE_CLASSIC_BACKGROUND_ID
                ? styles.backgroundSection__itemSelected
                : styles.backgroundSection__item
            }
          >
            <span className={styles.backgroundSection__classic}>Win95</span>
            <span className={styles.backgroundSection__name}>Classique</span>
          </button>
          {SOLITAIRE_BACKGROUNDS.map((background) => (
            <button
              key={background.id}
              type="button"
            ...
            >
              ...
              <img
                ...
                draggable={false}
                className={styles.backgroundSection__thumb}
              />
              <span className={styles.backgroundSection__name}>
                {background.label}
              </span>
            </button>
          ))}
