...
import {
  ...
  subscribeSolitaireBackground,
  type SolitaireBackground,
...
> = (props) => {
  ...
  const [background, setBackground] = useState<SolitaireBackground | null>(
    getStoredSolitaireBackground()
  );

  useEffect(
    () => subscribeSolitaireBackground((next) => setBackground(next)),
    []
  );
  ...
  return (
    <div className={styles.gameUIBorder} id="gameContainer">
      ...
      <CardDragLayer
        pilesContainer={pilesContainer as React.RefObject<HTMLDivElement>}
