  ...
  .superRefine((value, ctx) => {
    if (
      (value.reviewStatus === "EXCLUDED" ||
        (value.reviewStatus === "APPROVED" && !value.isEligible)) &&
    ...
    if (
      ...
      ctx.addIssue({
        code: "custom",
        path: ["trackId"],
        message: "Une vidéo approuvée et éligible doit être liée à une chanson.",
      });
    }

    if (
      EXCLUDED_YOUTUBE_VIDEO_TYPES.includes(
        value.videoType as (typeof EXCLUDED_YOUTUBE_VIDEO_TYPES)[number]
      ) &&
      value.isEligible
    ) {
      ctx.addIssue({
        code: "custom",
        path: ["isEligible"],
        message: "Ce type de vidéo ne peut pas être éligible au classement principal.",
      });
    }
  });
