...
const REFERENCE_ICONS: Record<string, BrandLinkMeta> = {
  url_wikipedia: { label: "Wikipedia", brand: "wikipedia", icon: SiWikipedia },
  url_chartmetric: { label: "Chartmetric", brand: "chartmetric", icon: SiChartdotjs },
  url_shazam: { label: "Shazam", brand: "shazam", icon: SiShazam },
};
...
export default async function ArtistProfilePage({ params }: Props) {
  ...
  const isDeceased = !!artist.is_deceased;
  ...
  const references = Object.entries(REFERENCE_ICONS)
    .filter(([key]) => artist[key])
    .map(([key, meta]) => ({ url: artist[key] as string, ...meta }));
    ...
    <>
      ...
      <main className="artist-profile">
        ...
        <div className="wrap">
          ...
          {references.length > 0 && (
            <section className="artist-profile__section artist-profile__section--links">
              <h2 className="artist-profile__section-title">Références & analyses</h2>
              <div className="artist-profile__links" aria-label="Références et analyses de l'artiste">
                {references.map((reference) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`artist-profile__brand-link artist-profile__brand-link--${reference.brand}`}
                      aria-label={`Consulter ${artist.name} sur ${reference.label}`}
                      title={reference.label}
                    >
                      <span className="artist-profile__brand-icon">
                        <Icon aria-hidden="true" />
                      </span>
                      <span className="artist-profile__brand-label">{reference.label}</span>
                    </a>
                  );
                })}
              </div>
          ...
          {isDeceased && (
            <section className="artist-profile__section artist-profile__section--deceased">
              <h2 className="artist-profile__section-title">🕯️ Étoiles éteintes</h2>
              <p className="artist-profile__deceased-note">
                Cet artiste nous a quittés, mais son héritage musical continue de illuminer la planète.
                Son nom figure dans notre hommage aux artistes disparus.
              </p>
              <div className="artist-profile__deceased-badges">
                <span className="badge-deceased">🕯️ Artiste décédé</span>
                <span className="badge-deceased">✨ Héritage musical</span>
                <span className="badge-deceased">🕯️ Étoile éteinte</span>
              </div>
            </section>
          )}
