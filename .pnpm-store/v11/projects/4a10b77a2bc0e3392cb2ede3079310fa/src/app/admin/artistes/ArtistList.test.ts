...
describe("filtres administratifs des artistes", () => {
  ...
  it("isole totalement les artistes exclus des autres compteurs", () => {
    const excluded = { ...COMPLETE_ARTIST, is_excluded: true, is_active: false };
    expect(matchesArtistFilter(excluded, "excluded")).toBe(true);
    expect(matchesArtistFilter(excluded, "all")).toBe(false);
    expect(matchesArtistFilter(excluded, "claimed")).toBe(false);
    expect(matchesArtistFilter(excluded, "incomplete")).toBe(false);
    expect(matchesArtistFilter(excluded, "missing_spotify")).toBe(false);
  });
});
