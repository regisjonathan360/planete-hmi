...
export function ArtistEditForm({
  ...
  initialGroups?: RelationshipOption[];
  initialMembers?: RelationshipOption[];
  ...
  const [form, setForm] = useState({
    ...
    url_wikipedia: (artist.url_wikipedia as string) ?? "",
    url_chartmetric: (artist.url_chartmetric as string) ?? "",
    url_shazam: (artist.url_shazam as string) ?? "",
    gender: (artist.gender as string) ?? "",
    is_deceased: (artist.is_deceased as boolean) ?? false,
    death_date: (artist.death_date as string) ?? "",
  });

  function update(field: string, value: string | boolean | string[] | null) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 800 }}>
      ...
      <Fieldset title="Identité">
        ...
        <Row>
          <div style={{ flex: 1 }}>
            <span style={labelStyle}>Sexe / type de profil</span>
            <p style={{ color: "var(--admin-muted)", fontSize: "0.76rem", margin: "0.25rem 0 0.55rem" }}>
              Utilisé pour le filtre public. Choisissez « Groupe » pour un orchestre ou collectif.
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem" }}>
              {[
                ...
                <button
                  ...
                  style={{
                    background: form.gender === option.value ? "var(--admin-accent)" : "transparent",
                    borderColor: form.gender === option.value ? "var(--admin-accent)" : "var(--admin-border)",
                    color: form.gender === option.value ? "#fff" : "var(--admin-muted)",
                  }}
                >
                  {option.label}
                </button>
      ...
      <Fieldset title="Statut vital">
        <Row>
          <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", flex: 1 }}>
            ...
            <span style={labelStyle}>🕯️ Artiste décédé</span>
          </label>
          {form.is_deceased && (
            <Field 
              label="Date de décès" 
        ...
        {form.is_deceased && (
          <p style={{ color: "var(--admin-warning)", fontSize: "0.76rem", margin: "0.5rem 0 0" }}>
            ⚠️ Cet artiste apparaîtra dans la section &quot;Étoiles éteintes&quot; sur la page publique des artistes et ne sera pas inclus dans la section anniversaire.
          </p>
        )}
      </Fieldset>
      ...
      <Fieldset title="Plateformes musicales">
        ...
        <Row><Field label="Chartmetric" value={form.url_chartmetric} onChange={(v) => update("url_chartmetric", v)} type="url" /></Row>
        <Row><Field label="Shazam" value={form.url_shazam} onChange={(v) => update("url_shazam", v)} type="url" /></Row>
      </Fieldset>
      ...
      {/* Autres plateformes */}
      <Fieldset title="Autres plateformes">
        <Row>
          <label style={{ display: "flex", flexDirection: "column", gap: "0.25rem", flex: 1, minWidth: 180 }}>
            <span style={labelStyle}>Wikipedia</span>
            <input
              type="url"
              value={form.url_wikipedia}
              onChange={(e) => update("url_wikipedia", e.target.value)}
              placeholder="https://fr.wikipedia.org/wiki/Nom_Artiste"
              style={inputStyle}
            />
            <span style={{ color: "var(--admin-muted)", fontSize: "0.72rem", marginTop: "0.2rem" }}>
              ✨ Extraction automatique : bio, date/lieu naissance, nom réel, genres, image
            </span>
          </label>
        </Row>
        <Row><Field label="Chartmetric" value={form.url_chartmetric} onChange={(v) => update("url_chartmetric", v)} /></Row>
        <Row><Field label="Shazam" value={form.url_shazam} onChange={(v) => update("url_shazam", v)} /></Row>
      </Fieldset>
      ...
      <Fieldset title="Enrichissement automatique">
        <EnrichmentPanel
          ...
          urls={{
            ...
            url_wikipedia: form.url_wikipedia,
            url_chartmetric: form.url_chartmetric,
            url_shazam: form.url_shazam,
          }}
          onWikipediaCollected={(result) => {
            setForm((previous) => ({
              ...previous,
              // La collecte complète uniquement les champs encore vides : une
              // correction faite par l'équipe reste toujours prioritaire.
              bio: previous.bio || result.description || "",
              birth_place: previous.birth_place || stringDetail(result.details.birth_place),
              real_name: previous.real_name || stringDetail(result.details.real_name),
              birth_date: previous.birth_date || normalizeWikipediaDate(result.details.birth_date_raw),
              career_start_year: previous.career_start_year || numberDetail(result.details.career_start_year),
        ...
        {form.artist_type === "group" ? (
          <>
            ...
            <RelationshipPicker
              ...
              searchPlaceholder="Rechercher un artiste à ajouter"
            />
          </>
        ) : (
          <>
            ...
            <RelationshipPicker
              ...
              kind="groups"
              artistId={artist.id as string}
              initialSelected={initialGroups}
              selectedIds={groupIds}
              onChange={setGroupIds}
              searchPlaceholder="Rechercher un groupe ou un orchestre"
            />
          </>
        )}
      </Fieldset>
...
function RelationshipPicker({
  ...
  kind: "groups" | "members";
  artistId: string;
  initialSelected: RelationshipOption[];
  selectedIds: string[];
  onChange: (ids: string[]) => void;
  searchPlaceholder: string;
}) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<RelationshipOption[]>([]);
  const [selectedOptions, setSelectedOptions] = useState<RelationshipOption[]>(initialSelected);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      void fetch(`/api/admin/artistes/search-relations?${params}`, {
  ...
  function add(option: RelationshipOption) {
  ...
  function remove(id: string) {
    ...
    setSelectedOptions((current) => current.filter((option) => option.id !== id));
  }

  const visibleResults = results.filter((option) => !selectedIds.includes(option.id));
    ...
    <div style={{ position: "relative" }}>
      <label style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
        ...
        <span style={{ color: "var(--admin-muted)", fontSize: "0.72rem" }}>
          Saisissez au moins 2 caractères. Seuls 15 résultats sont affichés.
        </span>
      </label>
      ...
      {query.trim().length >= 2 ? (
        ...
        >
          {loading ? (
            <p style={{ color: "var(--admin-muted)", fontSize: "0.8rem", margin: "0.5rem" }}>
              Recherche en cours…
            </p>
            ...
            visibleResults.map((option) => (
              ...
              >
                ...
                <small style={{ color: "var(--admin-muted)" }}>
                  {option.artist_type === "group" ? "Groupe" : "Artiste"}
                </small>
              </button>
            ))
          ) : (
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem", marginTop: "0.8rem" }}>
        {selectedOptions.filter((option) => selectedIds.includes(option.id)).map((option) => (
          <span
            key={option.id}
            style={{
              alignItems: "center",
...
function stringDetail(value: FieldResult["details"][string] | undefined): string {
  return typeof value === "string" ? value.trim() : "";
}

function numberDetail(value: FieldResult["details"][string] | undefined): string {
  return typeof value === "number" && Number.isInteger(value) && value >= 1800 && value <= 2100
    ? String(value)
    : "";
}

function normalizeWikipediaDate(value: FieldResult["details"][string] | undefined): string {
  const raw = stringDetail(value);
...
function normalizeWikipediaGenre(value: string | undefined): string {
...
const FIELD_LABELS: Record<string, { label: string; icon: string }> = {
  ...
  url_wikipedia: { label: "Wikipedia", icon: "📖" },
  url_chartmetric: { label: "Chartmetric", icon: "📊" },
  url_shazam: { label: "Shazam", icon: "🎵" },
};
...
function EnrichmentPanel({
  ...
  async function collectField(field: string) {
    ...
    try {
      ...
      setMetricSummaries(json.metricSummaries ?? []);
      if (field === "url_wikipedia" && !json.error) {
        onWikipediaCollected(json as FieldResult);
      }
      if (json.error) {
  ...
  async function collectAllFields() {
    ...
    try {
      ...
      setMetricSummaries(json.metricSummaries ?? []);
      const wikipedia = json.results?.url_wikipedia as FieldResult | undefined;
      if (wikipedia && !wikipedia.error) onWikipediaCollected(wikipedia);
