  ...
  return (
    ...
    >
      ...
      <div className="entry__actions">
        ...
        {entry.artistIsExcluded ? (
          <Link className="btn btn--sm btn--ghost" href="/admin/artistes?filter=excluded" title="Réintégration depuis la liste des artistes exclus">Exclu globalement</Link>
        ) : entry.isExcluded ? (
          <button className="btn btn--sm" disabled={busy} onClick={() => onAction(entry.entryId, "include")}>Réintégrer</button>
        ) : (
          <button className="btn btn--sm btn--danger" disabled={busy} onClick={() => onAction(entry.entryId, "exclude", { reason: "Non haïtien" })}>Exclure globalement</button>
        )}
      </div>
    </div>
  );
