...
export async function recomputeAdminEdition(
  ...
  // Défense supplémentaire pour les anciennes éditions : si un artiste a été
  // exclu globalement, l'entrée est retirée même si elle précédait le registre.
  const trackIds = entries.map((entry) => entry.track_id).filter((id): id is string => Boolean(id));
  if (trackIds.length) {
    const { data: excludedCredits, error: exclusionError } = await supabase
      .from("track_artists")
      .select("track_id, artists!inner(is_excluded)")
      .in("track_id", trackIds)
      .eq("artists.is_excluded", true);
    if (exclusionError) throw new Error(`Lecture des exclusions échouée: ${exclusionError.message}`);
    const excludedTrackIds = new Set((excludedCredits ?? []).map((credit) => credit.track_id as string));
    for (const entry of entries) {
      if (!entry.track_id || !excludedTrackIds.has(entry.track_id)) continue;
      entry.is_excluded = true;
      await supabase
        .from("chart_entries")
        .update({ is_excluded: true, exclusion_reason: "Artiste exclu globalement", filtered_position: null })
        .eq("id", entry.id);
    }
  }
