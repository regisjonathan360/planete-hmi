...
export async function getAdminChartData(
  ...
  const primaryByTrack = new Map<
    string,
    { artistId: string; name: string; status: HaitianStatus; isActive: boolean; isExcluded: boolean; imageUrl: string | null }
  >();
  if (trackIds.length) {
    const { data: credits } = await supabase
      .from("track_artists")
      .select("track_id, role, billing_order, artists(id, name, haitian_status, is_active, is_excluded, image_url)")
    ...
    (credits ?? []).forEach((c: unknown) => {
      const row = c as {
        track_id: string;
        artists: { id: string; name: string; haitian_status: HaitianStatus; is_active: boolean; is_excluded: boolean; image_url: string | null } | null;
      };
      ...
      if (!primaryByTrack.has(row.track_id)) {
        primaryByTrack.set(row.track_id, {
          ...
          isExcluded: row.artists.is_excluded,
  ...
  const entries: AdminChartEntry[] = rows
    .map((r): AdminChartEntry => {
      ...
      const eligible =
        !!primary && estStatutVerifie(primary.status) && primary.isActive && !primary.isExcluded;
      ...
      return {
        ...
        artistIsExcluded: primary?.isExcluded ?? false,
        isEligible: eligible,
  ...
  const artistIds = [
    ...new Set(entries.filter((e) => !e.artistIsExcluded).map((e) => e.primaryArtistId).filter((x): x is string => !!x)),
  ];
  if (artistIds.length) {
    const { data: artistRows } = await supabase
      .from("artists")
      .select("id, name, slug, haitian_status, is_active, is_excluded, confidence_score, image_url, tags")
      .in("id", artistIds);
    ...
    (artistRows ?? []).filter((a) => !a.is_excluded).forEach((a) => {
