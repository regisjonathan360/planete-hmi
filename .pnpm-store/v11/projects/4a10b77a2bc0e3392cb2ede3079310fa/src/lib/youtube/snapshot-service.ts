...
export class ComputeDraftService {
  ...
  async execute(ctx: StepContext): Promise<StepResult> {
    ...
    const performances = videos.map(video => {
      ...
      const input: YouTubeVideoPeriodInput = {
        video: {
          ...
          sourceTitle: video.sourceTitle ?? video.videoId,
    ...
    const ranked = rankYouTubeVideos(performances, 20);
    const videosByExternalId = new Map(videos.map((video) => [video.videoId, video]));

    await ctx.updateProgress(90, "compute_draft:writing");
    await ctx.assertActive();

    // Build entries with total_views for preview
    const entries: DraftEntry[] = ranked.map(performance => {
      const video = videosByExternalId.get(performance.videoId);
      if (!video) throw new Error(`Vidéo classée introuvable : ${performance.videoId}`);
      return {
        youtube_video_id: video.id,
        track_id: video.trackId,
        metric_value: performance.weeklyViews ?? 0,
        raw_artist_text: video.channelTitle ?? "Chaîne YouTube",
        raw_track_title: video.displayTitle ?? video.sourceTitle ?? video.videoId,
        delta_views: performance.weeklyViews ?? 0,
        delta_likes: performance.weeklyLikes ?? 0,
        delta_comments: performance.weeklyComments ?? 0,
        total_views: performance.totalViews ?? 0,
