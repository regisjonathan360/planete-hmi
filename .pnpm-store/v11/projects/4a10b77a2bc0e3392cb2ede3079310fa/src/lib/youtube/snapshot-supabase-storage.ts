...
export function createSnapshotStorage(): SnapshotStorage {
  ...
  return {
    async getEligibleVideos(): Promise<EligibleVideo[]> {
      // La chanson est facultative : le classement porte directement sur les vidéos.
      const { data, error } = await supabase
        .from("youtube_videos")
        .select(`
          id, video_id, channel_id, track_id, published_at, video_type,
          source_title, display_title, source_thumbnail_url, display_thumbnail_url,
          youtube_channels(channel_title)
        `)
        .eq("is_active", true)
      ...
      return data.map((row: Record<string, unknown>) => ({
        ...
        sourceThumbnailUrl: (row.source_thumbnail_url as string | null) ?? null,
        displayThumbnailUrl: (row.display_thumbnail_url as string | null) ?? null,
        channelTitle:
          ((Array.isArray(row.youtube_channels) ? row.youtube_channels[0] : row.youtube_channels) as { channel_title?: string } | null)?.channel_title
          ?? "Chaîne YouTube",
