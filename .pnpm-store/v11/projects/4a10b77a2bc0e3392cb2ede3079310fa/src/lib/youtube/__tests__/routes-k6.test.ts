...
describe("youtubeVideoEditorialInputSchema", () => {
  ...
  it("accepte une vidéo éligible sans chanson", () => {
    expect(youtubeVideoEditorialInputSchema.safeParse({ ...baseValid, trackId: "" }).success).toBe(true);
  });

  it("refuse un SHORT éligible dans le classement principal", () => {
...
describe("validateYouTubeDraft", () => {
  ...
  it("autorise une vidéo sans artiste ou chanson associé", () => {
    const entries = Array.from({ length: 20 }, (_, i) =>
      makeEntry(i === 0 ? { artistIsLinked: false, trackId: null } : {})
    );
    const result = validateYouTubeDraft({
      periodStart: "2026-07-14", periodEnd: "2026-07-21",
      publicPeriodLabel: "Test", entries,
  ...
  it("warning si moins de 20 vidéos", () => {
    const entries = Array.from({ length: 15 }, () => makeEntry());
    const result = validateYouTubeDraft({
      periodStart: "2026-07-14", periodEnd: "2026-07-21",
      publicPeriodLabel: "Test", entries,
    });
    expect(result.warnings.some(w => w.code === "LESS_THAN_20_VIDEOS")).toBe(true);
  });

  it("plusieurs vidéos éligibles avec la même référence de chanson restent indépendantes", () => {
