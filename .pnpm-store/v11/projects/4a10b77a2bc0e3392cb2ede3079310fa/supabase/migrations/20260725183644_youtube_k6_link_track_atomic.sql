-- =========================================================
-- K6 v3 â€” RPCs transactionnelles pour les routes administratives YouTube
--
-- 1. link_youtube_video_to_track : association atomique vidÃ©oâ€“chanson
--    - Verrouille la vidÃ©o avec SELECT ... FOR UPDATE
--    - Supprime TOUTES les anciennes associations (pas seulement l'ancienne canonique)
--    - Garantit une seule chanson canonique par vidÃ©o
--
-- 2. approve_youtube_video : approbation atomique complÃ¨te
--    - Une seule transaction pour tout le processus d'approbation
--    - Aucun Ã©tat partiel possible
--
-- 3. acquire_recalculate_lease : acquisition de lease pour le recalcul
--    - MÃªme clÃ© advisory que acquire_sync_lease
--
...
-- Une vidÃ©o possÃ¨de une seule chanson canonique, tandis qu'une chanson peut
-- toujours agrÃ©ger plusieurs vidÃ©os.
CREATE UNIQUE INDEX IF NOT EXISTS yt_assets_one_track_per_video_idx
  ON public.youtube_track_assets (youtube_video_id);

-- =========================================================
-- approve_youtube_video : approbation atomique complÃ¨te
--
-- RÃ©alise dans une seule transaction :
-- 1. Verrouillage de la vidÃ©o
-- 2. VÃ©rification vidÃ©o + chanson + type Ã©ligible
-- 3. Suppression associations contradictoires
-- 4. CrÃ©ation/mise Ã  jour de l'association canonique
-- 5. Mise Ã  jour youtube_videos (track_id, review_status, etc.)
-- =========================================================
...
BEGIN
  IF p_source_key IS NULL OR btrim(p_source_key) = '' THEN
    RAISE EXCEPTION 'p_source_key ne peut pas Ãªtre vide';
  END IF;
  IF p_period_key IS NULL OR btrim(p_period_key) = '' THEN
    RAISE EXCEPTION 'p_period_key ne peut pas Ãªtre vide';
  END IF;
  IF p_owner_token IS NULL OR btrim(p_owner_token) = '' THEN
    RAISE EXCEPTION 'p_owner_token ne peut pas Ãªtre vide';
  END IF;
  IF p_lease_duration_seconds IS NULL OR p_lease_duration_seconds < 10 OR p_lease_duration_seconds > 3600 THEN
    RAISE EXCEPTION 'p_lease_duration_seconds doit Ãªtre entre 10 et 3600';
  END IF;
  ...
  IF FOUND THEN
    -- Active lease held by another owner â†’ refuse
    ...
    -- Expired/released or same owner â†’ take over
...
-- =========================================================
-- update_youtube_video_editorial : mise Ã  jour atomique des champs Ã©ditoriaux
--
-- RÃ©alise dans une seule transaction :
-- 1. Verrouillage de la vidÃ©o
-- 2. Validation chanson si fournie
-- 3. Validation cohÃ©rence statut/type/Ã©ligibilitÃ©/chanson
-- 4. Mise Ã  jour association si la chanson change
-- 5. Mise Ã  jour champs Ã©ditoriaux
-- =========================================================
