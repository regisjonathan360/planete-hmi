...
SET display_name = 'Top vidéos YouTube HMI',
    chart_context = 'Vidéos classées individuellement selon leurs nouvelles vues hebdomadaires',
    updated_at = now()
...
-- Conserve l'ancien endpoint d'approbation, mais rend la chanson facultative.
CREATE OR REPLACE FUNCTION public.approve_youtube_video(
  p_youtube_video_id uuid, p_track_id uuid, p_video_type text,
  p_review_reason text, p_reviewed_by uuid
)
RETURNS TABLE(success boolean, message text)
LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
BEGIN
  IF p_youtube_video_id IS NULL OR p_video_type IS NULL
    THEN RETURN QUERY SELECT false, 'missing_params'::text; RETURN; END IF;
  IF p_reviewed_by IS NULL
    THEN RETURN QUERY SELECT false, 'missing_reviewer'::text; RETURN; END IF;
  IF p_review_reason IS NULL OR length(btrim(p_review_reason)) < 10
    THEN RETURN QUERY SELECT false, 'review_reason_too_short'::text; RETURN; END IF;
  IF p_video_type NOT IN (
    'OFFICIAL_MUSIC_VIDEO','OFFICIAL_AUDIO','OFFICIAL_LYRIC_VIDEO',
    'OFFICIAL_VISUALIZER','OFFICIAL_ANIMATION'
  ) THEN RETURN QUERY SELECT false, 'ineligible_video_type'::text; RETURN; END IF;
  ...
  ELSE
    ...
    INSERT INTO public.youtube_track_assets
      (track_id,youtube_video_id,asset_role,is_primary,is_eligible,linked_by,linked_at)
    VALUES (p_track_id,p_youtube_video_id,'primary',false,true,p_reviewed_by,clock_timestamp())
    ON CONFLICT (track_id,youtube_video_id) DO UPDATE SET
      is_eligible=true, linked_by=EXCLUDED.linked_by, linked_at=EXCLUDED.linked_at;
  END IF;

  UPDATE public.youtube_videos SET
    track_id=p_track_id, review_status='APPROVED', video_type=p_video_type,
    is_eligible=true, review_reason=p_review_reason, reviewed_by=p_reviewed_by,
...
REVOKE ALL ON FUNCTION public.approve_youtube_video(uuid,uuid,text,text,uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.approve_youtube_video(uuid,uuid,text,text,uuid)
  TO service_role;
